...
class node:
    def __init__(self, dat):
        try:
            self.parents = ast.literal_eval(dat.parents_id)
            self.children = ast.literal_eval(dat.children_id)
        except(AttributeError):
            self.parents = []
            self.children = []


# Base Buccheim Drawtree Init method adapted for OSS datastructure
class DrawTree(object):
    ...
    def get_lmost_sibling(self):
        if not self._lmost_sibling and self.parents and self != \
                self.parents.children[0]:
                    self._lmost_sibling = self.parents.children[0]
